...
func (s *Svc) QuoteInquiry(ctx context.Context, req *qpb.QuoteInquiryRequest) (*qpb.QuoteInquiryResponse, error) {
	...
	if err := s.validators[pn].QuoteInquiryValidate(ctx, req); err != nil {
		return nil, err
	}

	res, err := s.quote.QuoteInquiry(ctx, req)
	if err != nil {
		switch t := err.(type) {
		case *perahub.Error:
...
func (*WISEVal) QuoteInquiryValidate(ctx context.Context, req *qpb.QuoteInquiryRequest) error {
