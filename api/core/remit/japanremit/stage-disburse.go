...
type cacheDisburse struct {
	...
	StageResp perahub.JPRInquireResponseBody `json:"stage_response"`
}
...
func (s *Svc) StageDisburseRemit(ctx context.Context, rmt core.Remittance) (rres *core.Remittance, cErr error) {
	...
	res, err := s.ph.JPRInquire(ctx, perahub.JPRInquireRequest{
	...
	if res.Message != perahub.JapanRemitAwaitPayment {
