...
func (s *Svc) StageCreateRemit(ctx context.Context, r core.Remittance) (*core.RemitResponse, error) {
	return nil, coreerror.NewCoreError(codes.Unavailable, "service not available for instacash")
}

func handleInstaCashError(err error) *coreerror.Error {
	switch err.(type) {
	case *perahub.Error:
		...
		if pErr.GRPCCode == codes.InvalidArgument {
			...
			if strings.Contains(pErr.Msg, "ICTC number is Invalid") && pErr.Code == "0" {
				return coreerror.NewCoreError(codes.NotFound, coreerror.MsgControlNumberNotFound)
			}

			// Status = 400, Code = 2, Msg = Paid
			if strings.Contains(pErr.Msg, "Transaction status is Not outstanding") {
