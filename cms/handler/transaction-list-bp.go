...
import (
	...
	tpb "brank.as/petnet/gunk/drp/v1/bills-payment"
...
type transactionBPTempData struct {
	BillsPayments            []*tpb.BillsPayment
	UserInfo                 *User
	CompanyName              string
...
func (s *Server) getBPTransactionList(w http.ResponseWriter, r *http.Request) {
	...
	template := s.templates.Lookup("transaction-list-bp.html")
	if template == nil {
	...
	switch sbc {
	case "bpamount":
		sbcv = tpb.SortByColumn_GrossAmount
	case "fee":
		sbcv = tpb.SortByColumn_Fee
	case "commission":
		sbcv = tpb.SortByColumn_Commission
	case "dateprocessed":
		sbcv = tpb.SortByColumn_TransactionCompletedTime
	}
	...
	lr := &tpb.BillsPaymentTransactListRequest{
		Limit:           limitPerPage,
		Offset:          offset,
		SortOrder:       sbv,
		SortByColumn:    sbcv,
	...
	var res *tpb.BillsPaymentTransactListResponse
	if apiEnvType == "production" {
		res, err = s.drpLV.BillsPaymentTransactList(ctx, lr)
	} else {
		res, err = s.drpSB.BillsPaymentTransactList(ctx, lr)
	}
	if err != nil {
		logging.WithError(err, log).Error("listing remits failed")
	}
	newReq := &spbl.GetPartnerListRequest{
		Status:      spb.PartnerStatusType_ENABLED.String(),
		ServiceName: psvc.ServiceType_BILLSPAYMENT.String(),
	}
	...
	var total int32
	tempData := transactionBPTempData{
		BillsPayments:            []*tpb.BillsPayment{},
	...
	if res != nil && res.BillsPayments != nil {
		tempData.BillsPayments = res.GetBillsPayments()
		total = res.Total
	}

	if len(tempData.BillsPayments) > 0 {
