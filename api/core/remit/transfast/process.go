...
func (s *Svc) ProcessRemit(ctx context.Context, r core.ProcessRemit) (*core.ProcessRemit, error) {
	...
	if err != nil {
		...
		logging.WithError(err, log).Error("get remit cache db error for tf")
...
func (s *Svc) disburse(ctx context.Context, r core.ProcessRemit) (rres *core.ProcessRemit, cErr error) {
	...
	t := time.Now()
	if _, err = s.ph.TFPayout(ctx, perahub.TFPayoutRequest{
		// internal/static
		...
		RcvStateID:  cres.Result.RcvStateID,
		RcvCityID:   cres.Result.RcvCityID,
		RefNo:       cres.Result.RefNo.String(),
		...
		RcvOccupationID: json.Number(creq.Receiver.Employment.OccupationID),
		...
		KYCVerified:    creq.Receiver.KYCVerified,
		IDType:         creq.Receiver.PrimaryID.IDType,
		ControlNo:      creq.ControlNo,
		RmtReasonID:    json.Number(creq.Receiver.SendingReasonID),
		ProofOfAddress: creq.Receiver.ProofOfAddress,
		DsaCode:        dsaCode,
		DsaTrxType:     dsaTrxType,
	}); err != nil {
		logging.WithError(err, log).Error("tf payout error")
