...
type RTAMetrobankPaymentRequest struct {
	...
	BeneZipCode                 string `json:"bene_zip_code"`
}

type RTAMetrobankPaymentResponse struct {
	Code     int                       `json:"code"`
	Message  string                    `json:"message"`
	Result   RTAMetrobankPaymentResult `json:"result"`
	BankCode string                    `json:"bank_code"`
}

type RTAMetrobankPaymentResult struct {
	Message string `json:"message"`
}

func (c *Client) RTAMetrobankPayment(ctx context.Context, req RTAMetrobankPaymentRequest) (*RTAMetrobankPaymentResponse, error) {
	res, err := c.phService.RtaPost(ctx, c.getUrl("metrobank-rta/payment"), req)
	if err != nil {
		return nil, err
	}

	mbp := &RTAMetrobankPaymentResponse{}
	if err := json.Unmarshal(res, mbp); err != nil {
		return nil, err
	}
	return mbp, nil
}
