...
func NewInfluxDBClient(config *viper.Viper) (*Influxdb, error) {
	...
	batchSize := config.GetInt("influxdb.batchSize")
	bucket := config.GetString("influxdb.bucket")
	organization := config.GetString("influxdb.organization")
	authToken := config.GetString("influxdb.token")

	grpcMeasure := config.GetString("influxdb.grpcmeasure")
	httpMeasure := config.GetString("influxdb.httpmeasure")
...
func (r *Influxdb) WriteSpan(ctx context.Context, tags map[string]string, fields map[string]interface{}) {
	...
	pt := write.NewPoint(span.measurement, tg, span.fields, time.Now()).
		SortFields().SortTags()
	r.w.WritePoint(pt)
}
