...
type cacheDisburse struct {
	...
	StageResp perahub.RMInquireResponseBody `json:"stage_response"`
}
...
func (s *Svc) StageDisburseRemit(ctx context.Context, rmt core.Remittance) (rres *core.Remittance, cErr error) {
	...
	rmt.Agent.LocationName = "INFORMATION TECHONOLOGY" // todo: this will be given by petnet
	rmt.Agent.LocationCode = rmt.Agent.LocationID
	rmt.Agent.AgentID = "84424911" // todo: this will be given by petnet
	rmt.Agent.AgentCode = rmt.Agent.LocationID
	if phmw.GetTerminalID(ctx) != "" {
		rmt.Agent.DeviceID = phmw.GetTerminalID(ctx)
	}
	res, err := s.ph.RMInquire(ctx, perahub.RMInquireRequest{
		...
		AgentCode:    rmt.Agent.LocationID,
		BranchCode:   rmt.Agent.LocationID,
		LocationCode: rmt.Agent.LocationCode,
		CurrencyCode: rmt.DestAmount.CurrencyCode(),
	})
	if err != nil {
	...
	if res.Msg != perahub.RMAwaitPayment {
	...
	amt, err := currency.NewAmount(res.Result.PnplAmt, res.Result.CurrencyCode)
	if err != nil {
