...
type cacheDisburse struct {
	...
	StageResp perahub.UNTInquireResponseBody `json:"stage_response"`
}
...
func (s *Svc) StageDisburseRemit(ctx context.Context, rmt core.Remittance) (rres *core.Remittance, cErr error) {
	...
	res, err := s.ph.UNTInquire(ctx, perahub.UNTInquireRequest{
	...
	if res.Result.PrincipalAmount == "" {
		logging.WithError(err, log).Error("Principle amount is empty")
		return nil, coreerror.NewCoreError(codes.InvalidArgument, "Principle amount is empty")
	}
