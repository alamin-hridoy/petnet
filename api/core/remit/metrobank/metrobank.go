...
func (s *Svc) StageCreateRemit(ctx context.Context, r core.Remittance) (*core.RemitResponse, error) {
	return nil, coreerror.NewCoreError(codes.Unavailable, "service not available for metrobank")
}

func handleMetroBankError(err error) *coreerror.Error {
	switch err.(type) {
	case *perahub.Error:
		...
		if pErr.GRPCCode == codes.InvalidArgument {
			if pErr.Code == "4" || strings.Contains(pErr.Msg, "Transaction does not exist") {
