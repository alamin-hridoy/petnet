...
function createToken(wrapper, value) {
    ...
    close.innerHTML += '<img src="../images/cross-new.svg">';
    const closeImg = close.firstChild;
    close.addEventListener("click", removeToken)
    closeImg.addEventListener("click", removeToken)
...
function clearAutocompleteList(select) {
    ...
    autocomplete_list.classList.remove("h-44");
...
function populateAutocompleteList(select, query, dropdown = false) {
    ...
    const autocomplete_list = wrapper.querySelector(".autocomplete-list");
    autocomplete_list.classList.add("h-44");
    autocomplete_list.innerHTML = "";
...
function removeToken(e) {
    ...
    const value_to_remove = e.target.parentNode.dataset.option;
    const wrapper = e.target.parentNode.parentNode.parentNode;
    const input_search = wrapper.querySelector(".selected-input");
    const dropdown = wrapper.querySelector(".dropdown-icon");
    ...
    let SaveDraft = document.getElementById("SaveDraft");
    let SaveContinue= document.getElementById("SaveContinue");
    let vv = document.querySelectorAll(".selected-wrapper");
    if(vv.length == 0 || vv.length < 0) {
        SaveDraft.classList.add("hidden");
        SaveContinue.classList.add("hidden");
    }
    getMultiSelectVals();
