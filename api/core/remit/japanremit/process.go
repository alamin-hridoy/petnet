...
func (s *Svc) ProcessRemit(ctx context.Context, r core.ProcessRemit) (*core.ProcessRemit, error) {
	...
	if err != nil {
		...
		logging.WithError(err, log).Error("get remit cache db error for jpr")
...
func (s *Svc) disburse(ctx context.Context, r core.ProcessRemit) (rres *core.ProcessRemit, cErr error) {
	...
	t := time.Now()
	if _, err = s.ph.JPRPayout(ctx, perahub.JPRPayoutRequest{
		...
		logging.WithError(err, log).Error("jpr payout error")
