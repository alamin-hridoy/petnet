...
func (s *Svc) DisableServiceRequest(ctx context.Context, req *spb.ServiceStatusRequestRequest) (*emptypb.Empty, error) {
	...
	if err := s.st.DisableSvcRequest(ctx, storage.ServiceRequest{
		OrgID:     req.GetOrgID(),
		Partner:   req.GetPartner(),
		SvcName:   req.GetSvcName(),
		UpdatedBy: req.GetUpdatedBy(),
	}); err != nil {
		logging.WithError(err, log).Error("disable request failed")
		return nil, err
	}
