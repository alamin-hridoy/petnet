...
func TestPerahubRemitConfirmTransaction(t *testing.T) {
	t.Parallel()

	st := newTestStorage(t)
	tests := []struct {
		name        string
		in          PerahubRemitConfirmRequest
		expectedReq PerahubRemitConfirmRequest
		want        *PerahubRemitConfirmResponseBody
		wantErr     bool
	}{
		{
			name: "Success",
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			s, m := newTestSvc(t, st)
			got, err := s.PerahubRemitconfirm(context.Background(), test.in)
			if (err != nil) != test.wantErr {
				t.Errorf("PerahubRemitconfirm() error = %v, wantErr %v", err, test.wantErr)
				return
			}
			var newReq PerahubRemitConfirmRequest
