...
type MetricsClient struct {
	...
	// TODO: add batched metrics sending which will collect metrics
	// for x seconds, then batch send them all?
}
...
func (m MetricsClient) createSingleMetric(metricType, metricName string, metricValue *monitoringpb.TypedValue, labels map[string]string) error {
	...
	*/
	...
	metricInterval := &monitoringpb.TimeInterval{
		EndTime: &googlepb.Timestamp{Seconds: time.Now().UTC().Unix()},
	}

	switch metricType {
	case "CUMULATIVE":
		metricKind = metricpb.MetricDescriptor_CUMULATIVE
		metricInterval.StartTime = &googlepb.Timestamp{Seconds: m.startTime.Unix()}
	case "GAUGE":
...
func (m MetricsClient) GaugeString(metricName string, metricValue string, labels map[string]string) {
