...
type cacheDisburse struct {
	...
	StageResp perahub.BPInquireResponseBody `json:"stage_response"`
}
...
func (s *Svc) StageDisburseRemit(ctx context.Context, rmt core.Remittance) (rres *core.Remittance, cErr error) {
	...
	rmt.Agent.LocationName = "IT Department" // todo: this will be given by petnet
	res, err := s.ph.BPInquire(ctx, perahub.BPInquireRequest{
	...
	resList := map[string]bool{
		perahub.BPAwaitPayment:  true,
		perahub.BPAwaitPaymentD: true,
	}
	...
	amt, err := currency.NewAmount(res.Result.PnplAmt, "PHP")
	if err != nil {
