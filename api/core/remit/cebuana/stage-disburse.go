...
type cacheDisburse struct {
	...
	StageResp perahub.CEBInquireResponseBody `json:"stage_response"`
}
...
func (s *Svc) StageDisburseRemit(ctx context.Context, rmt core.Remittance) (rres *core.Remittance, cErr error) {
	...
	res, err := s.ph.CEBInquire(ctx, perahub.CEBInquireRequest{
	...
	if res.Result.ResultStatus != perahub.CEBAwaitPayment {
	...
	amt, err := currency.NewAmount(res.Result.PnplAmt.String(), res.Result.Currency)
	if err != nil {
