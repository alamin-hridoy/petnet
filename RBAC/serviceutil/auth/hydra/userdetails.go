...
func extractString(extra map[string]interface{}, key string) (string, error) {
	resultData, ok := extra[key]
	if !ok {
		return "", fmt.Errorf("missing %q in auth token", key)
	}
	result, ok := resultData.(string)
	if !ok {
		return "", fmt.Errorf("%q in auth token is not a string", key)
	}
	return result, nil
}

func (ud *UserDetails) authViaToken(extra map[string]interface{}) (*UserInfo, error) {
