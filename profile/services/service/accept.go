...
func (s *Svc) AcceptServiceRequest(ctx context.Context, req *spb.ServiceStatusRequestRequest) (*emptypb.Empty, error) {
	...
	if err := validation.ValidateStruct(req,
		...
		logging.WithError(err, log).Error("invalid arguments")
		return nil, status.Error(codes.InvalidArgument, err.Error())
	}

	if err := s.st.AcceptSvcRequest(ctx, storage.ServiceRequest{
		OrgID:     req.GetOrgID(),
